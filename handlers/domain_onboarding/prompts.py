...
def planning_system_prompt() -> str:
    return (
        ...
        "subdirection_plans:[{subdirection_id,name_zh,name_en,scope,include_terms:[string],"
        "exclude_terms:[string],research_questions:[string],search_queries:[{query,role_hint,"
        "path_id,priority}]}]. If the input is Chinese, translate "
        ...
        "exactly 3 expected_subdirections and exactly 3 matching subdirection_plans. Each branch must "
        "have a standard English research name, a non-overlapping scope, 2-4 include terms, optional "
...
def section_system_prompt(section: str, language: str) -> str:
    ...
    instructions = {
        ...
        "landscape": (
            ...
            "workflow steps and exactly 3 research questions. Copy each subdirection_id and name from "
            "research_plan.subdirection_plans. A branch may use only its own selected_paper_ids; never "
            "borrow evidence from another branch merely to fill a field. Keep each explanatory field to "
            "1-2 sentences."
        ),
        "learning_path": (
...
def full_generation_system_prompt(language: str) -> str:
    return (
        ...
        "paper mappings. Subdirections must copy IDs and names from subdirection_plans and may cite only "
        "their selected_paper_ids. Subdirections must include actionable tasks, prerequisites, techniques, datasets, "
