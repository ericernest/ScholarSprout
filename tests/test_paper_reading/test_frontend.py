...
class PaperReadingFrontendTests(unittest.TestCase):
    ...
    def test_history_selection_toolbar_and_vertical_maps_restore_expected_ui(self) -> None:
        ...
        self.assertIn("normalizeConversationContexts", shared)
        self.assertIn('paper_reading:${context.paper_id || context.id || ""}', shared)
    ...
    def test_agent_answers_use_markdown_renderer(self) -> None:
        ...
        self.assertIn('conversationScrollTarget = "latest-assistant"', shared_javascript)
        self.assertIn("scrollMessageToTop(latestAssistant)", shared_javascript)
        self.assertIn("if (requestedCard && !wasRendered)", shared_javascript)
        self.assertIn("messages.scrollTop = Math.max(0, previousScrollTop)", shared_javascript)
        self.assertIn("syncWorkspaceUrl()", javascript)
        self.assertIn('params.set("session_id", state.sessionId)', javascript)
