...
class DomainOnboardingFrontendTests(unittest.TestCase):
    ...
    def test_workspace_consumes_snapshot_sse_and_paper_import(self) -> None:
        ...
        self.assertIn('["paper-score-v2", "paper-score-v3"].includes(paper.score_version)', script)
        self.assertIn('"主题相关 · 65%"', script)
        self.assertIn('"检索覆盖 · 15%"', script)
        self.assertIn('"时效性 · 10%"', script)
        self.assertIn('"信息完整度 · 10%"', script)
        self.assertIn("引用数仅作元数据展示，不参与推荐度", script)
