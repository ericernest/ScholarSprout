...
class DomainOnboardingFrontendTests(unittest.TestCase):
    def test_workspace_route_and_assets_exist(self) -> None:
        ...
        self.assertIn('data-scholarsprout-entry="domain-onboarding"', response.text)
    ...
    def test_workspace_consumes_snapshot_sse_and_paper_import(self) -> None:
        ...
        self.assertNotIn('<span>引用数</span>', script)
        self.assertNotIn('class="paper-score"', script)
        self.assertNotIn("综合推荐度", script)
        ...
        self.assertIn("Array.isArray(data.papers) ? data.papers : []", script)
        self.assertNotIn("Array.isArray(data.evidence_papers) ? data.evidence_papers : []", script)
        self.assertIn("证据论文未混入清单", script)
        self.assertIn('paperFilter: "all"', script)
        self.assertNotIn('state.paperFilter === "focus"', script)
        self.assertNotIn('label: `重点 ${focusCount}`', script)
        self.assertIn('recent_survey: "近期综述"', script)
        self.assertIn('survey_reference: "综述引用"', script)
        self.assertIn('`<span class="chip">引用 ${escapeHtml(Number(paper.citation_count))}</span>`', script)
        ...
        self.assertIn("const papers = paperIndex(data)", script)
        self.assertIn("function paperIndex(data)", script)
        self.assertIn("...(data?.evidence_papers || [])", script)
        self.assertIn("...(data?.papers || [])", script)
        ...
        self.assertIn("function isInternalLandscapeLabel", script)
        self.assertIn("function mergeLandscapeItems", script)
        self.assertIn("if (byName.size) return [...byName.values()]", script)
        self.assertIn("isInternalLandscapeLabel(name)", script)
    ...
    def test_chat_reloads_persisted_history_after_returning_to_page(self) -> None:
        ...
        self.assertIn("window.setInterval(reloadWhenEnteringChat, 650)", script)
        self.assertIn("patchPersistedMessages(history)", script)
