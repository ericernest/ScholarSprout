...
async function appendStyles(source: Document): Promise<void> {
  ...
  await Promise.all(links.map((sourceLink) => new Promise<void>((resolve, reject) => {
    ...
    link.dataset.paperauroraLegacy = props.surfaceId;
    link.onload = () => resolve();
  ...
  override.dataset.paperauroraLegacy = `${props.surfaceId}-overrides`;
...
async function appendScripts(source: Document): Promise<void> {
  ...
  for (const sourceScript of scripts) {
    await new Promise<void>((resolve, reject) => {
      ...
      script.dataset.paperauroraLegacy = props.surfaceId;
...
async function loadSurface(): Promise<void> {
  try {
    ...
    document.documentElement.dataset.paperauroraReady = props.surfaceId;
    status.value = "ready";
  } catch (error) {
...
onBeforeUnmount(() => {
  ...
  delete document.documentElement.dataset.paperauroraReady;
});
</script>
