...
- 默认数据目录为 `~/.scholarsprout/`，可在 `/settings` 配置向导或 `~/.scholarsprout/config.json` 的 `storage.data_dir` 修改。`SCHOLARSPROUT_DATA_DIR` 仍作为部署环境的最高优先级覆盖项。领域入门 job 默认也使用同一个 `research.sqlite3`；只有显式配置 `DOMAIN_ONBOARDING_JOB_DB` 时才使用独立文件。
