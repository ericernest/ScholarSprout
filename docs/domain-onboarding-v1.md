...
请求与画像
→ STORM-lite 领域规划
→ survey / foundational / method / evaluation / frontier 分角色检索
→ 分角色候选池、分路径 RRF 与全局 MMR 排序
→ 发展阶段提纲
...
- `llm.py`：统一的结构化模型调用与 JSON 语法边界；
- `structured_response.py`：有界候选定位、类型感知归一化与歧义拒绝；
- `response_contracts.py`：规划、阶段、主模块和修复响应的声明式契约；
...
所有领域入门 LLM JSON 都先经过同一套结构适配器，再进入 Pydantic 和业务校验。
适配器最多检查两层对象包装，根据必填字段和实际数据类型选择候选，而不是按对象
字段数量猜测。每类响应只声明允许的 wrapper、字段别名和安全类型转换，因此可以
兼容 `result/data/output`、模型新增的普通包装层以及 `stage/steps/path` 等已知别名。

允许的转换仅限契约明确声明的形式，例如单个对象转成单元素对象数组、单元素对象
数组转成对象，以及从对象数组中删除非对象项。字符串不会被当成数组逐字符展开；
类型错误的同名字段也不会覆盖类型正确的别名。两个不同对象同等匹配时直接拒绝，
避免静默选错内容。适配过程只记录契约名、来源路径和转换事件，不记录 Prompt、
API Key 或完整模型输出。

结构适配只修复表现形式。论文 ID 白名单、阶段顺序、最低内容数量、证据绑定和最终
Pydantic 校验仍由原业务层执行，适配器不会创建论文、论述或占位内容。
...
- 每个查询最多 10 篇论文；
- 最多保留 40 篇候选、选择 12 篇；
- 排序权重：相关性 0.70、时效 0.20、多样性 0.10；引用数只展示，不参与评分；
...
每个来源保持原有的查询上限，但会在不同来源间轮换查询窗口，使所有目标论文角色至少进入一个真实数据源。
...
误认为实时检索。
Ranker 在截取候选上限时同时轮询论文角色与来源，避免某个早期查询或高产来源占满候选池。
规划器会保留标准英文领域名、缩写与相关技术词，生成带 `role_hint`、`path_id` 和优先级的独立查询。
检索结果保留命中的查询角色和路径；去重时合并这些来源信息。候选论文先在每条路径内排序，再用 RRF 与全局质量分融合；MMR 前额外保留每个可用角色的高分候选，最后综合新颖度、路径和角色覆盖选取论文。
...
规划查询中的常见模型格式 `methods`、`high`、`medium`、`low` 会在 Schema 边界
确定性归一化为 `method` 和数值优先级，避免局部格式错误导致整份英文领域规划回退。
Survey 推荐必须以 `translated_domain` 中不含中文的英文研究术语为锚点；缺失该锚点时
不会发送中英混合的宽泛查询，也不会从发现词或证据 Survey 绕过，而是返回
`survey_degraded_no_result` 并记录 `missing_standard_english_domain`。
