# ScholarSprout（科研萌芽）前端重构说明

## 结论

v1 使用 Vue 3 + TypeScript + Vite 的多页面构建，产物仍由 FastAPI 静态托管。后端 API、SSE、数据库、Agent 与部署命令保持不变。六个公开入口由 typed surface registry 统一声明；每个入口先加载对应的 v1 功能页面，再按顺序装载原样式、功能脚本和 ScholarSprout 兼容层，因此重构期间不牺牲已经可用的交互，也不改变上一版页面配色和布局。
